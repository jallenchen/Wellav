...
public class TeamCreateHelper {
        ...
        NIMClient.getService(TeamService.class).createTeam(fields, TeamTypeEnum.Normal, "",
                ...
                new RequestCallback<Team>() {
                    ...
                    public void onSuccess(Team team) {
                        ...
                        Toast.makeText(DemoCache.getContext(), R.string.create_team_success,
                    ...
                    public void onFailed(int code) {
                        ...
                        if (code == 801) {
                            ...
                            Toast.makeText(DemoCache.getContext(), R.string.create_team_failed,
                                    Toast.LENGTH_SHORT).show();
                        }
    ...
    public static void createAdvancedTeam(final Context context, List<String> memberAccounts) {
        ...
        String teamName = "XX的家";

        // 创建群
        TeamTypeEnum type = TeamTypeEnum.Advanced;
        HashMap<TeamFieldEnum, Serializable> fields = new HashMap<>();
        fields.put(TeamFieldEnum.Name, teamName);
        NIMClient.getService(TeamService.class).createTeam(fields, type, LauncherApp.getInstance().getFamily().getNickName()+"的家庭组",
                memberAccounts).setCallback(
                new RequestCallback<Team>() {
                    @Override
                    public void onSuccess(Team t) {
                        Log.e(TAG, "create team success, team id =" + t.getId() + ", now begin to update property...");
                        SysConfig.getInstance().setMyTeam(t);
                        PreferenceUtils.getInstance().saveSharePreferences(MsgKey.Key_TeamID, t.getId());
    ...
    private static void onCreateSuccess(final Context context, final Team team) {
        ...
        Toast.makeText(DemoCache.getContext(), R.string.create_team_success, Toast.LENGTH_SHORT).show();
