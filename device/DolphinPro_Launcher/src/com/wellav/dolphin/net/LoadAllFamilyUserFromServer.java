...
public class LoadAllFamilyUserFromServer {
	String mBody;
	String mRequest;
	Handler handler;
	int Code = -1;
	...
	public LoadAllFamilyUserFromServer(String body) {
		mBody = body;
				...
				if (msg.what == Code && dataCallBack != null) {
					...
					if (infos != null) {
						LauncherApp
								.getInstance()
								.getDBHelper()
								.saveFamilyUserInfoList(infos,
										LauncherApp.getInstance());
						LauncherApp.getInstance().setFamilyUsers(infos);
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.END_TAG:
					if (response.getName().equalsIgnoreCase("n:Users")
						...
						if (familyUserInfo.getLoginName().equals(SysConfig.uid)) {
							familyUserInfoes.add(0, familyUserInfo);
						} else {
							familyUserInfoes.add(familyUserInfo);
						}
						if (Util.getManger(familyUserInfo.getAuthority()) == true) {
							LauncherApp.getInstance().setFamilyManager(
									familyUserInfo);
						}
						familyUserInfo = null;
					}
