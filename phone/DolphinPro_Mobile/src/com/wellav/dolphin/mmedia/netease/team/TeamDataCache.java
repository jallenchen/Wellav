...
import com.wellav.dolphin.mmedia.netease.SimpleCallback;
...
public class TeamDataCache {
	private static String TAG ="TeamDataCache";
    ...
    public String getTeamMemberDisplayName(String tid, String account) {
        if (account.equals(SysConfig.uid)) {
            return "我";
        }
    ...
    public String getDisplayNameWithoutMe(String tid, String account) {
        ...
        String alias ="测试员1";// NimUserInfoCache.getInstance().getAlias(account);
        if (!TextUtils.isEmpty(alias)) {
            return alias;
        }

        return "测试员2";//NimUserInfoCache.getInstance().getUserName(account);
    }
