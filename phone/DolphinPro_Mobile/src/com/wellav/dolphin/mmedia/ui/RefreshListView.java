...
public class RefreshListView extends ListView implements OnScrollListener {
	...
	private void initFooterView() {
		footerView = View.inflate(getContext(), R.layout.freshlistview_footer, null);
	...
	private void initHeaderView() {
		headerView = View.inflate(getContext(), R.layout.freshlistview_header, null);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if(currentState==REFRESHING||isLoadingMore)
...
//			Log.e("", "dy="+downY+"my="+moveY+"diff="+diff);
	...
	private void refreshHeaderView() {
		...
		case DOWN_PULL_REFRESH: // 下拉刷新状态
			tvState.setText(R.string.pull_to_refresh);
			ivArrow.startAnimation(downAnimation); // 执行向下旋转
			break;
		case RELEASE_REFRESH: // 松开刷新状态
			tvState.setText(R.string.loosen_refresh);
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		...
		if ((scrollState == SCROLL_STATE_IDLE || scrollState == SCROLL_STATE_FLING)&&this.canScrollVertically(-1)) {
			// 判断当前是否已经到了底部
...
//				Log.i(TAG, "加载更多数据");
	...
	public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
		...
		if (headerView.getPaddingTop()==-headerViewHeight&&getLastVisiblePosition() == (totalItemCount - 1)) {
	...
	public void hideHeaderView0()
	{
//		headerView.setPadding(0, -headerViewHeight, 0, 0);
		ObjectAnimator anim=ObjectAnimator.ofFloat(headerView, "alpha", 1.0f,0f);
		...
		anim.addUpdateListener(new AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				...
				headerView.setPadding(0,(int) -((1-(Float) animation.getAnimatedValue())*headerViewHeight), 0, 0);
			}
		});
		anim.start();
	}
