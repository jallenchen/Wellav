...
public class NameUtils {
	...
	public static boolean remoteNamesAdd(String name) {// 需要加入逗号是被分拣
		...
		for (int i = 0; i < userIds.length; i++) {
			if (mRemoteNames.contains(userIds[i])) {
				//Log.i(TAG, "remoteNamesAdd 想将" + userIds[i] + "加入远端名单，但已经包含");
				break;
			} else if(userIds[i].equals(SysConfig.uid)){
				continue;
			}else {
			//	Log.i(TAG, "remoteNamesAdd 将" + userIds[i] + "加入了远端名单");
	...
	public static boolean remoteNamesRemove(String name) {
		if (remoteNamesContain(name)) {
			//Log.i(TAG, "remoteNamesRemove 远端名单包含" + name + ",移除成功");
	...
	public static void remoteNamesClean() {
		//Log.i(TAG, "remoteNamesClean 清除远端用户列表");
		mRemoteNames.clear();
	}

	public static String userInfo2LoginName(List<UserInfo> users,
			int callingmeber) {
	...
	public static List<FamilyUserInfo> loginName2UserInfo(
		...
		for (int i = 0; i < stringList.size(); i++) {
			Users.add(SQLiteManager.getInstance().geFamilyUserInfoLoginName(
					stringList.get(i)));
		}
	...
	public static FamilyUserInfo loginName2UserInfo(String stringList, int audio) {
		FamilyUserInfo Users;
		if (stringList == null) {

		}
		Users = SQLiteManager.getInstance().geFamilyUserInfoLoginName(
				stringList);
		Users.setAudioState(audio);
