...
public class StickyGridHeadersGridView extends GridView implements
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		if (!isLoading&&(scrollState == SCROLL_STATE_IDLE || scrollState == SCROLL_STATE_FLING)
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		if (mHeaderChildBeingPressed) {
			...
			if (tempHeader != null) {
				tempHeader.dispatchTouchEvent(transformEvent(ev,
						mMotionHeaderPosition));
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mMotionHeaderPosition == NO_MATCHED_HEADER
				...
				if (tempHeader != null) {
					if (tempHeader.dispatchTouchEvent(transformEvent(ev,
							mMotionHeaderPosition))) {
				...
				if (header != null) {
					...
					if (mTouchMode == TOUCH_MODE_DOWN
						...
						if (!mDataChanged) {
							 ...
							 * have triggered on the callback handler.
							 */
							mTouchMode = TOUCH_MODE_TAP;
	...
	private void measureHeader() {
		...
		if (mHeadersIgnorePadding) {
			...
			widthMeasureSpec = MeasureSpec
					.makeMeasureSpec(getWidth() - getPaddingLeft()
	...
	private void scrollChanged(int firstVisibleItem) {
		...
		if (mCurrentHeaderId != newHeaderId) {
			swapStickiedHeader(mAdapter.getHeaderView(selectedHeaderPosition,
	...
	void attachHeader(View header) {
		...
		try {
			...
			Method method = View.class
					.getDeclaredMethod("dispatchAttachedToWindow",
	...
	void detachHeader(View header) {
		...
		try {
			Method method = View.class
					.getDeclaredMethod("dispatchDetachedFromWindow");
			method.setAccessible(true);
